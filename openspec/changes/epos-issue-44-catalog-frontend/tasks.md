...
- [ ] 10.2 A workflow that packs `examples/go-house/` with `epos pack` and publishes it to `ghcr.io/gaarutyunov/skills/` (**D10**). Use `epos push` if epos#43 has merged, otherwise `oras cp --from-oci-layout-path "$(epos store path)"`. `GITHUB_TOKEN` is sufficient and **no new secret is needed — but the job needs `permissions: packages: write` and a ghcr login**; `ci.yml` declares `contents: read`, so the default is not enough. **Blocked until epos#42 merges** — that is the only task in this change that is.
