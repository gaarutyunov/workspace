...
- [ ] 8.4 `playground.CompileWithShaping(sdlSrc, query, vars, sqlSide)` alongside `Compile`/`CompileWithMaxDepth`, returning the SQL, the bind values, the projection and the derived result-set shape (*k* columns assembled in Go versus one `response` column assembled in PostgreSQL).
- [ ] 8.5 Playground tab: a Go-side / SQL-side toggle over one SDL, query and variables, showing the SQL each strategy emits, running the real compiled Go (design D8).
...
- [ ] 8.10 **Wire the toggle to execution** (design D8, amended after gopgql#31 merged). `playground.ShapeSQLSide` decodes the SQL-side `response` column; `playground.ShapeParity` shapes both result sets and compares the two `shape.Encode` outputs; `gopgqlShapeParity` exposes it. The worker protocol takes a list of queries so both statements run against one database in one round trip. `ExampleShapingSeed` fans out 2×2 so the two result sets visibly differ. The panel's "generated SQL, not results" wording is replaced with what the comparison actually is — canonical encodings, not the database's bytes.
- [ ] 8.11 Coverage for the wiring: `playground/parity_test.go` on the host (including a deliberate disagreement, so the verdict is a check and not a constant); a `test/seed` case asserting the fan-out counts *and* the parity against a real `postgres:19beta2`; a `docs/e2e` case asserting the panel runs both and reports agreement in a real browser.
