...
longer deferred.** It lands in M0 as generic methods on `*Registry`, on the
**Go 1.27** floor (design D8, D17).

**D8-A is decided.** The spec had recommended package-level generic functions on
Go 1.24, with the 1.27 method form costed where it would land; the owner answered
on 2026-08-04 — *"I don't care, use 1.27. Build dependencies from source if
necessary."* Everything below is written that way: `go 1.27` plus `toolchain
go1.27rc2` (0.1), generic methods (0.4a), and the from-source golangci-lint build
as real M0 work (0.4d, 0.6), reverted at 11.4 and documented at 13.6. No task
below is conditional on that answer any more.
...
- [ ] 0.1 `go mod init github.com/gaarutyunov/goga`; **`go 1.27` *plus* `toolchain go1.27rc2`** (design D17, the owner's answer to D8-A). **Both lines, and the `toolchain` line is not optional** — measured, not assumed: with a bare `go 1.27` and the default `GOTOOLCHAIN=auto` the build fails with *"go: downloading go1.27.0 … download go1.27.0 for darwin/amd64: toolchain not available"*, because it tries to fetch a GA release that does not exist. Only rc1 and rc2 are published; with `toolchain go1.27rc2` present the same build succeeds. The `toolchain` line moves to each successive RC and is deleted at GA. **What sets this floor is the registry's generic methods** (0.4a) and nothing else: the **generic type alias** `type Option[S any] = goga.Option[S]` — how a module re-exports the root option type without minting a distinct named type, since an adapter's `registry.Option[settings]` is *not* assignable to a module-level `type Option = goga.Option[settings]` — gates at **`go1.23`** (*"generic type alias requires go1.23 or later"* at `go 1.22`), and `reflect.TypeFor` needs 1.22. *(A plain alias to an already-instantiated generic type — `type PgxAdapter = Adapter[DB, settings]` — is **not** the gated feature and compiles at 1.22.)* Those are recorded because they are what the floor would fall to if the registry ever reverted to package-level functions — not because anything here may build below 1.27. Go's module rule propagates the floor into **every consumer**, so 13.6 has to record that `GOTOOLCHAIN=auto` silently switches a 1.26 developer onto the RC while `GOTOOLCHAIN=local` fails hard.
...
- [ ] 0.4a **`goga/registry`** (design D8), keyed on a plain adapter **name** — not a URL scheme, because goga picks adapters at build time in the composition root and encoding a compile-time fact as a runtime string buys nothing here. Four exported forms, all **generic methods on `*Registry`** (design D8-A, the owner's decision — this is what sets the `go 1.27` floor in 0.1): `(r *Registry) Register[P, S any](name, ctor func(context.Context, S) (P, error)) error` — both type parameters inferred from the constructor even in the method form, so `S` may stay **unexported** in the adapter's own package; `(r *Registry) Open[P any](ctx, name, raw Settings) (P, error)` — the config-driven path, `P` result-only and therefore explicitly instantiated; `(r *Registry) Provide[P, S any]` returning the typed handle `Adapter[P, S]` and `Register`'s error, whose `Open(ctx, raw, opts ...Option[S])` is fully static and is what keeps variadic options type-safe; and `(r *Registry) Names()` for diagnostics. **`Adapter[P, S].Open` is untouched by the method form** — its type parameters belong to the type, not the method — so the call site consumers actually use is identical either way. **The constructor signature is fixed at `func(context.Context, S) (P, error)` and goga holds it** — adapter construction is I/O in every adapter-bearing module, and a ctx accepted at `Open` with nowhere to go cannot reach an OTLP dial or an MCP bind. Verified on `go1.27rc2`: neither the leading `ctx` nor the receiver disturbs inference — both type parameters are still inferred from a bare `r.Register(name, ctor)`. Precedence is **config first, options second**. `Register` **returns an error** on a duplicate name rather than panicking (design D8): with the registry an injected value there is no `init()`, so a duplicate is detected while ordinary startup code runs, where a returned error beats a panic. `New` still panics on a **nil decoder**, because that is a wiring mistake with no sensible recovery. **The registry is an injected value with no package-level default**, so a test building its own registry cannot collide with another test's. `Registry` therefore carries a `sync.RWMutex` — with no `init()`-time global, registration and lookup can race. Errors name types with `reflect.TypeFor[P]()`, never `%T` on a zero value, which prints `<nil>` for a nil interface. It is **very nearly a leaf**: the standard library, `reflect`, and the root `goga` package alone — the last only so `registry.Option[S]` can be a generic type **alias** of `goga.Option[S]` rather than a second, incompatible declaration (two identical declarations are distinct named types, and an adapter's option would then be unusable with `goga.Apply`). Root `goga` is itself a leaf, so no cycle is possible. It carries **no `Instrumentation`**, so `registry` → `telemetry` → `registry` still cannot form; the resolve span belongs to the module that owns the port. *(Changed from "standard library only" — that claim is no longer true, and any depguard rule written against it needs the root-`goga` exception.)*
  ...
  - Tests cover the duplicate-name **error**, the nil-decoder panic in `New`, the unknown name, a successful typed open, settings decoded into an adapter's unexported struct **including a `koanf:`-tagged key that does not match its field name**, an option for the wrong adapter failing to **compile** on the `Adapter[P, S]` path, the wrong-port failure from `Open[P]`, and `Register`/`Open`/`Names` under `-race` with concurrent access.
  - **Adapter settings are exported for wire by an alias, not by exporting the struct**: `type PgxAdapter = registry.Adapter[*pgxpool.Pool, settings]` in the adapter package. A downstream provider cannot name `Adapter[P, settings]` when `settings` is unexported (*"name settings not exported by package …"*), but it can name the exported alias — as a provider parameter, a struct field and a second provider's input. Compiler-checked on 1.27rc2 in the generic-method form, and previously on 1.24 in the superseded package-level form.
...
- [ ] 0.4d **Build golangci-lint from source, against `golang.org/x/tools` v0.48.0.** This is the *"build dependencies from source if necessary"* half of the owner's answer to D8-A, and it is real M0 work, not a contingency — without it D18's linter-every-milestone rule cannot be met at all on the 1.27 floor. Three measurements pin the recipe: golangci-lint **v2.7.2 as shipped refuses to run on a Go 1.27 target** (*"the Go language version (go1.26) used to build golangci-lint is lower than the targeted Go version (1.27rc2)"*); **rebuilt on go1.27rc2 it still fails** — *"cannot decode internal/goarch, export data version 4 is greater than maximum supported version 2"* — because it pins `x/tools` **v0.39.0**, and `staticcheck v0.6.1` fails identically; **only the rebuild against `x/tools` v0.48.0 reports `0 issues`** on generic-method code, and a custom `x/tools/go/analysis` analyzer on v0.48.0 parses *and* fully type-checks a generic method. So the bump is one line and it is load-bearing. `go-lint` (0.6) consumes this build instead of the upstream prebuilt action, and **11.4 is the task that reverts it** once upstream ships a release on a new enough `x/tools` (design D17).
...
- [ ] 0.6 The three composite actions goga's own CI needs now: `setup-go` (Go version defaults to what `go.mod` says, which on the 1.27 floor means the action must install `go1.27rc2` — a stock `setup-go` with no RC available is a hard failure under `GOTOOLCHAIN=local`), `go-lint` (gofmt gate, `go vet`, golangci-lint via **the from-source build of 0.4d, not the upstream prebuilt action** — the released binary cannot lint a Go 1.27 target at all), `go-test` (race, atomic coverage, summary and artifact). Actions SHA-pinned internally; projects pin **goga** and nothing else. **`go-lint` must pass the project's build tags**: a file behind `//go:build wireinject` is excluded from the default build and no linter sees it, so M9's hand-written injector ships unlinted unless the tag is named.
...
**scaffolds** are 0.4b–0.4c; the actions are 0.6, and the linter toolchain M0
now has to build for itself is 0.4d. **Part 6 does not apply** — there is nothing here to migrate a
...
- [ ] 1.2 Exporter selection uses the **shared `goga/registry`**, injected via `WithExporterRegistry` — not a per-module table and not `Table[P]`, both dropped (design D8). The **module keeps its own typed surface** over that shared mechanism (design D8, corrected after the audit): `RegisterTraceExporter` / `RegisterMetricExporter` / `RegisterLogExporter` take a ctx-carrying constructor and delegate to `r.Register`, and there is no exported lookup — resolution goes through the registry's `Open` method. A duplicate name returns an **error** per registry, not a panic (D8). Standard names delegate to `contrib/exporters/autoexport`, which mcp-anything already depends on; house names are additive. An unknown name fails at startup naming the supported values rather than silently disabling telemetry.
...
- [ ] 1.8 `For` resolves through OTel's **global delegating providers** (`otel.Tracer` / `otel.Meter`), never by snapshotting a concrete one, and creates its instruments lazily. Before `Setup` those globals are no-ops, so a *library* (gopgql) can use goga modules without configuring telemetry and telemetry appears the moment the consuming binary calls `Setup`. This is load-bearing, and it now rests on **construction order** rather than on `init()`: modules obtain their `Instrumentation` when they are **constructed**, and a composition root routinely constructs some of them — a registry, an adapter, a config loader — **before** `Setup` installs the real providers. A handle that snapshotted a no-op provider at construction would leave exactly those paths permanently unobserved while every test passed. *(The previous premise — package-level `var` tables populated from `init()` — is gone with the global registry; the conclusion is unchanged, per design D6.)*
...
- [ ] 6.7 Transports resolve through the **shared `goga/registry`**, injected rather than module-owned (design D8): `stdio` (default), streamable `http`, `sse`, each contributed by the composition root through `mcp.RegisterTransport` over the registry's `Provide` method (design D8) and constructed via `func(context.Context, S) (P, error)`. The unknown-name error names what is registered and points at the missing `Provide` call.
...
- [ ] 11.4 **Retire 0.4d's from-source golangci-lint build.** Drop it and return `go-lint` to the upstream prebuilt action as soon as golangci-lint ships a release built on a new enough `golang.org/x/tools` (v0.48.0 or later) to read Go 1.27 export data. Until then the from-source build stays; this task is a scheduled revisit, not an optional cleanup, because a from-source linter is a maintenance cost every milestone pays (design D17).
...
- [ ] 13.6 Record the **Go version requirement** prominently, in goga's README and in the skill (D17, and the API-conventions capability requires exactly this): adopting **any** goga package requires **Go 1.27** and moves the adopting project onto a **pre-GA toolchain**, because Go's module rule propagates the floor into every consumer. State both consequences plainly rather than by implication — under the default `GOTOOLCHAIN=auto` a developer on 1.26.4 is switched onto `go1.27rc2` silently, and under `GOTOOLCHAIN=local` the build fails outright (*"go.mod requires go >= 1.27rc2 (running go 1.26.4; GOTOOLCHAIN=local)"*) until `go1.27rc2` is installed. Confirm no module has raised the floor further, and that the `toolchain` directive still names a published RC.
...
*(`goga/registry` was here in the previous revision, deferred until Go shipped
generic methods. It is no longer deferred: the spike the owner asked for showed
the registry never needed them — it compiled on stock `go1.26.4` at language
version `go 1.22` — and the owner then decided D8-A in favour of the generic
method form anyway. It lands in **M0** as task 0.4a, as methods on `*Registry`,
on the Go 1.27 floor. Design D8 records what was proven and the one part of the
owner's formulation that does not compile: `OpenWith[P, S]` type-checks but
leaves `S` unchecked against the adapter name.)*
...
- [ ] C.2 When it does start: `Component`, `Ref[T]`, `Deployer`, and deployer selection through the **shared `goga/registry`** (design D8) rather than a module-owned table. **`Ref[T]`'s `T` is constrained to an interface, enforced by the local deployer at registration** — a distributing deployer returns a generated stub, never the concrete struct the local one stored, so `Ref[*myComponent]` is a reference that passes tests and fails in production. `Get` is a checked assertion returning a typed error; the type parameter saves the caller writing the assertion, it does not remove it.
