...
After a repo lands under `projects/`, register it with the machine-wide gortex daemon so the graph/MCP code tools can query it: `gortex track projects/<repo>` for the base clone, and `gortex track --as-worktree projects/<repo>/.worktrees/<branch>` for **each worktree** — gortex does **not** auto-index worktrees (a worktree is untracked in the base repo, so a plain `gortex track <repo>` misses it; verified empirically). The graph tools return nothing for a path no tracked repo covers; re-tracking an already-tracked path is a harmless no-op. This workspace itself is registered for the gortex MCP via `.mcp.json` + `.claude/settings.json` (committed); hooks live in the git-ignored `.claude/settings.local.json`. The workspace repo is **never tracked/indexed** — only the project clones under `projects/` are. A committed `.gortex.yaml` excludes `*` as a guard so that even an accidental `gortex track` of this repo indexes zero source files.
