...
- [x] 5.9 The **tables** half turned on over a history that never created tables is a sentinel error at generate time, with a message naming a fresh `--dir`. Nothing is written when it fires (D4a). Implemented in gopgql `f031a32` / `18a6bf4`: `Ownership.check` gained the `hasHistory && !h.NoTables && !o.Tables` case, returning `HalfConflictError{Half: TablesHalf, Adopting: true}`, and the CLI's `tablesAdoptGuidance` names the fresh `--dir` and the `--no-tables` escape hatch. Covered end to end against a real database by `test/split`'s `TestAdoptingTheTablesHalfIsRefusedBeforeAnythingIsWritten`, which demonstrates the hazard the refusal prevents — teardown committed, `ALTER TABLE … ADD COLUMN` failed, database left with no property graph — before asserting the refusal writes nothing and leaves the graph live.
