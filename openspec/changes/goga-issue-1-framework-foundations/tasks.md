...
- [ ] 17.6 **Resolve the guidance contradiction** (design D13): the spf13 `go` skill — now **merged and on `main`** at `.agents/skills/go/SKILL.md` — rejects `internal/`-layered packages, while `go-project-scaffold` and `skill-test`'s `AGENTS.md` prescribe them. This is a workspace task, not goga's code to change; goga stays layout-agnostic (D1).
