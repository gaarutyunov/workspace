...
- [ ] 0.1 `go mod init github.com/gaarutyunov/goga`; **`go 1.24`** (design D17). **Two specified constructs set a real language floor, and it is below 1.24** — measured, not assumed. The **generic type alias** `type Option[S any] = goga.Option[S]` is how a module re-exports the root option type without minting a distinct named type (an adapter's `registry.Option[settings]` is *not* assignable to a module-level `type Option = goga.Option[settings]`, because those are two different named types). Its gate is **`go1.23`**: at `go 1.22` the compiler says *"generic type alias requires go1.23 or later"*, and at `go 1.23` it is clean. `reflect.TypeFor` needs 1.22. *(A plain alias to an already-instantiated generic type — `type PgxAdapter = Adapter[DB, settings]` — is **not** the gated feature and compiles at 1.22; only aliases carrying their own type parameters are.)* **So 1.24 is chosen for support, not for syntax**: it is the oldest release still in upstream support at v1, and a framework should not hold consumers below the security-patched range. Do not lower it to 1.23 on the strength of the alias gate — generic aliases were only enabled by default from 1.24, having required `GOEXPERIMENT=aliastypeparams` before that, so 1.23 would build here and fail on a stock 1.23 toolchain. **If the owner answers D8-A with 1.27**, this line becomes `go 1.27` **plus `toolchain go1.27rc2`** — a bare `go 1.27` breaks under `GOTOOLCHAIN=auto`, which tries to fetch a GA release that does not exist — and the README then has to record that `GOTOOLCHAIN=auto` silently switches a 1.26 developer onto the RC while `GOTOOLCHAIN=local` fails hard. Go's module rule propagates the floor into every consumer, so this one line is the whole cost of that answer.
...
- [ ] 0.4a **`goga/registry`** (design D8), keyed on a plain adapter **name** — not a URL scheme, because goga picks adapters at build time in the composition root and encoding a compile-time fact as a runtime string buys nothing here. Four exported forms, all package-level generic **functions**: `Register[P, S any](r, name, ctor func(context.Context, S) (P, error))` — both type parameters inferred from the constructor, so `S` may stay **unexported** in the adapter's own package; `Open[P any](ctx, r, name, raw Settings) (P, error)` — the config-driven path, `P` result-only and therefore explicitly instantiated; `Provide[P, S any]` returning the typed handle `Adapter[P, S]`, whose `Open(ctx, raw, opts ...Option[S])` is fully static and is what keeps variadic options type-safe; and `Names(r)` for diagnostics. **The constructor signature is fixed at `func(context.Context, S) (P, error)` and goga holds it** — adapter construction is I/O in every adapter-bearing module, and a ctx accepted at `Open` with nowhere to go cannot reach an OTLP dial or an MCP bind. Verified: the leading `ctx` does **not** disturb inference — both type parameters are still inferred from a bare `Register(r, name, ctor)`. Precedence is **config first, options second**. `Register` panics on a duplicate name: registering one name twice in one registry is a programming error. **The registry is an injected value with no package-level default**, so the panic is scoped to that registry and a test building its own is isolated by construction. `Registry` therefore carries a `sync.RWMutex` — with no `init()`-time global, registration and lookup can race. Errors name types with `reflect.TypeFor[P]()`, never `%T` on a zero value, which prints `<nil>` for a nil interface. It is **very nearly a leaf**: the standard library, `reflect`, and the root `goga` package alone — the last only so `registry.Option[S]` can be a generic type **alias** of `goga.Option[S]` rather than a second, incompatible declaration (two identical declarations are distinct named types, and an adapter's option would then be unusable with `goga.Apply`). Root `goga` is itself a leaf, so no cycle is possible. It carries **no `Instrumentation`**, so `registry` → `telemetry` → `registry` still cannot form; the resolve span belongs to the module that owns the port. *(Changed from "standard library only" — that claim is no longer true, and any depguard rule written against it needs the root-`goga` exception.)*
...
- [ ] 1.2 Exporter selection uses the **shared `goga/registry`**, injected via `WithExporterRegistry` — not a per-module table and not `Table[P]`, both dropped (design D8). Adapters are contributed by the composition root with `registry.Provide`, so there is no `RegisterTraceExporter` / `RegisterMetricExporter` / `RegisterLogExporter` triple and no exported lookup. Duplicate names panic per registry. Standard names delegate to `contrib/exporters/autoexport`, which mcp-anything already depends on; house names are additive. An unknown name fails at startup naming the supported values rather than silently disabling telemetry.
...
- [ ] 6.7 Transports resolve through the **shared `goga/registry`**, injected rather than module-owned (design D8): `stdio` (default), streamable `http`, `sse`, each contributed by the composition root with `registry.Provide` and constructed via `func(context.Context, S) (P, error)`. The unknown-name error names what is registered and points at the missing `Provide` call.
